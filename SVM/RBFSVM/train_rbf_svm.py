"""
RBF Kernel SVM 训练 - Letter Recognition Dataset
...
# ========== 1. 加载数据 ==========
print("=" * 60)
print("RBF Kernel SVM - Letter Recognition")
print("=" * 60)
...
rbf_svm = SVC(
    kernel='rbf',
    C=1.0,
    gamma='scale',  # 默认使用 1/(n_features * X.var())
    random_state=42,
    verbose=True
)
...
# ========== 5. 预测 ==========
print("\n>>> 预测中 ...")
y_pred_train = rbf_svm.predict(X_train_scaled)
...
model_path = os.path.join(model_dir, "rbf_svm_model.pkl")
scaler_path = os.path.join(model_dir, "rbf_svm_scaler.pkl")

joblib.dump(rbf_svm, model_path)
