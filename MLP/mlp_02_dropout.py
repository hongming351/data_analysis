"""
模型 2：带 Dropout 正则的 MLP
=============================
结构：输入层(16) → 隐藏层(128, ReLU, Dropout) → 隐藏层(64, ReLU, Dropout) → 输出层(26, Softmax)
- 在隐藏层之间加入 Dropout 层抑制过拟合
- 与基础 MLP 做对照实验
"""
...
def train_model(model, train_loader, test_loader, criterion, optimizer, scheduler=None):
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if test_acc > best_test_acc:
            ...
            best_epoch = epoch
            patience_counter = 0
            torch.save(model.state_dict(), os.path.join(SAVE_DIR, 'dropout_mlp_best.pth'))
        else:
            patience_counter += 1
...
def plot_history(history, save_path):
    ...
    ax1.set_ylabel('Loss')
    ax1.set_title('Training Loss (Dropout)')
    ax1.legend()
    ...
    ax2.set_title('Test Accuracy (Dropout)')
    ax2.legend()
...
def main():
    print(f'{"="*60}')
    print('模型 2：带 Dropout 正则的 MLP')
    print(f'{"="*60}')
    print(f'结构: {INPUT_SIZE} → {HIDDEN1_SIZE}(ReLU,Dropout={DROPOUT_RATE}) → {HIDDEN2_SIZE}(ReLU,Dropout={DROPOUT_RATE}) → {NUM_CLASSES}')
    ...
    model.load_state_dict(torch.load(os.path.join(SAVE_DIR, 'dropout_mlp_best.pth')))
    y_true, y_pred = evaluate_model(model, X_test_t, y_test_t)

    plot_history(history, os.path.join(SAVE_DIR, 'dropout_mlp_curve.png'))
